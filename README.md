...
Openutau Yaml Dictionaries for Arpasing, En VCCV, Xsampa phonemizers, En Enunu, and En EnunuX/EnunuX to support Japanese, Korean, Chinese, and French (more languages to support soon)
 ...
 #### 📍 When using these dictionaries with Arpasing banks I recommend using my [Syllable-Based ARPAsing Phonemizer](https://github.com/Cadlaxa/Syllable-Based-ARPAsing-Phonemizer) because these dictionaries contains not so common **`[C V]`**, **`[V C]`**, and specially **`[C C]`** aliases, also it has a fallback feature for missing aliases and phonemes such as `[ax]`, `[dx]`, and `[zh]`
